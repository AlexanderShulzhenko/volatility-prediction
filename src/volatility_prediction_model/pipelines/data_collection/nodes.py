    ...
    for year in [2021, 2022, 2023]:
        ...
        for month in trange(1, 13):
            ...
            klines = client.futures_historical_klines(
                "BTCUSDT", interval, f"{year}-{month}-01T00:00:00Z", f"{year}-{month}-{num_days[month]}T23:59:00Z"
            )
...
def get_transactions_by_timestamps(t1: datetime, t2: datetime, symbol: str, verbose: bool = False) -> pd.DataFrame:
    """
    ...
    trades = client.get_aggregate_trades(startTime=int(t1_stamp), endTime=int(t2_stamp), symbol=symbol)
    ...
    try:
        ...
        while df["Time"].iloc[-1] < t2:
            ...
            trades = client.get_aggregate_trades(fromId=str(last_trade_id), symbol=symbol, limit=1000)
            num_calls += 1
...
def get_trades_data(master_list: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    for i in trange(len(master_list)):
        ...
        # check if empty
        if len(trades) > 0:
            trades_cleaned = trades.drop(
                columns=["TradeID", "FirstTradeID", "LastTradeID", "BestMatchFlg", "Amount", "Time"]
            )
